-- نظام الشلة - إصلاح مشكلة التكرار
-- ============================================
-- المشكلة: البونص والنقاط بتتضاف حتى لو الطالب حل نفس التحدي قبل كده
-- الحل: التحقق من المشاركة السابقة في نفس التحدي
-- ============================================

-- الجزء 1: دالة المراقبة (بدون تغيير)
-- ============================================
...
-- الجزء 2: دالة إنهاء التحدي وتوزيع المكافآت (محسّنة)
-- ============================================
...
BEGIN
    ...
    -- 1. إضافة نقاط للشلة (لو نجح التحدي) - مرة واحدة فقط
    ...
    -- 2. توزيع البونص الشخصي على المشاركين (فقط لو أول مرة يشاركوا في هذا التحدي)
    ...
    LOOP
        ...
        -- التحقق: هل هذا أول مشاركة للطالب في هذا التحدي؟
        -- نعد عدد المشاركات السابقة لنفس الطالب في نفس التحدي
        DECLARE
        ...
        BEGIN
            ...
            -- لو أكتر من مشاركة واحدة، يعني الطالب حل التحدي قبل كده
            IF v_participation_count > 1 THEN
                CONTINUE; -- تخطي هذا الطالب (مش هياخد بونص)
            END IF;
        END;
    ...
    -- 3. تحديث حالة التحدي (مرة واحدة فقط)
    UPDATE squad_exam_challenges
    SET status = 'completed'
    WHERE id = p_challenge_id;

    -- 4. إرسال رسالة النتيجة النهائية (مرة واحدة فقط)
    IF v_is_success THEN
        IF v_all_solved THEN
            INSERT INTO squad_chat_messages (squad_id, challenge_id, text)
            VALUES (
...
-- الجزء 3: Trigger لمنع التحديات المتعددة
-- ============================================
...
-- الجزء 4: دالة إنهاء يدوي (للزرار)
-- ============================================
