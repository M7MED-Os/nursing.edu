-- نظام الشلة - النسخة النهائية
-- ============================================
-- المنطق:
-- 1. الأفراد: ياخدوا البونص في كل تحدي (حتى لو نفس الامتحان)
-- 2. الشلة: تاخد نقاط مرة واحدة فقط لكل امتحان (أول تحدي بس)
-- ============================================

-- 1. إضافة UNIQUE constraint (لمنع تسجيل نفس الطالب في نفس التحدي مرتين)
-- ============================================
...
-- 3. دالة إنهاء التحدي (محسّنة - فحص التكرار للامتحان)
-- ============================================
...
BEGIN
    ...
    IF v_is_success AND v_is_first_time_exam THEN
        ...
        WHERE id = v_squad_id;
    ELSE
        v_squad_points := 0; -- للاستخدام في الرسالة
    END IF;
